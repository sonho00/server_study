...
size_t Worker::TryFetchGlobal(size_t batch_size = 256)
{
    ...
    if (!lock.owns_lock() || ctx.global_queue_.empty())
        return 0;
...
{
    if (victim.queue_tail_ - victim.queue_head_ == 0)
        return 0;
...
void Worker::RunComplex()
{
    while (true) {
        ...
        if (task_did_ > 0) {
            if (ctx.pending_tasks_.fetch_sub(task_did_, std::memory_order_acq_rel) == task_did_) {
