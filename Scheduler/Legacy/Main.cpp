...
int MakeTask() {
	...
	for (int i = 0; i < ctx.num_threads_; ++i) {
		for (int j = 0; j < 50; ++j) {
			Task* task = new Task([i] { workers[i].DummyTask(i * 50 + 250); });
			ctx.global_queue_.push_back(task);
		}
	}
	return ctx.num_threads_ * 50;
}
...
void RunTasks() {
...

	{
		...
		ctx.task_done_.wait(lock, [] {
			return ctx.pending_tasks_.load(std::memory_order_acquire) == 0;
		});
	}
	ctx.stop_ = true;
...
int main() {
	...
	double avg =
		std::accumulate(elapsedTimes.begin(), elapsedTimes.end(), 0.0) /
		elapsedTimes.size();
	double min_val =
