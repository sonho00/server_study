...
enum class Task { NONE, RECV, SEND, ACCEPT, CNT };

class MagicBuffer {
   public:
	MagicBuffer() = default;
	MagicBuffer(size_t size = 1 << 16) : size_(size) {
		SYSTEM_INFO sysInfo;
		GetSystemInfo(&sysInfo);
		if (size_ % sysInfo.dwAllocationGranularity != 0) {
			NetUtils::PrintError(
				"Buffer size must be a multiple of the system's "
		...
		if (!MapViewOfFileEx(hMap_, FILE_MAP_ALL_ACCESS, 0, 0, size_,
			...
			throw std::runtime_error(
				"Failed to map view of file to first half");
		}
		...
		if (!MapViewOfFileEx(hMap_, FILE_MAP_ALL_ACCESS, 0, 0, size_,
							 buffer_ + size_)) {
			throw std::runtime_error(
				"Failed to map view of file to second half");
		}
	}
