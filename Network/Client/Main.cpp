...
int main() {
	...
	std::thread sendThread([&client, chatPacket, movePacket]() {
		while (true) {
			if (!client.SendPacket(&chatPacket.header)) {
				std::cerr << "Failed to send packet to server." << std::endl;
				break;
			}

			if (!client.SendPacket(&movePacket.header)) {
				std::cerr << "Failed to send packet to server." << std::endl;
				break;
			}
	...
	std::thread receiveThread([&client, &buffer]() {
		while (true) {
			if (!client.ReceivePacket(buffer, sizeof(PACKET_HEADER))) {
			...
			if (!client.ReceivePacket(buffer + sizeof(PACKET_HEADER),
				...
				std::cerr << "Failed to receive full "
							 "packet from server."
						  << std::endl;
				break;
			}

			if (!client.HandlePacket(header)) {
				std::cerr << "Failed to handle "
							 "packet from server."
						  << std::endl;
				break;
			}
